...
void poly_uniform(poly *a, const unsigned char *seed)
{
  ...
  const __m512i modulus8 = _mm512_set1_epi32(PARAM_Q);
  const __m512i modulus16 = _mm512_set1_epi16(PARAM_Q);


  uint32_t offset = 0;
  __mmask16 good = 0;

  while(ctr < PARAM_N-32)
  {
    __m512i tmp0, tmp1;

    tmp0 = _mm512_loadu_si512((__m512i *)&buf[pos]);

    // shift candidates into [0,q) range by
    // subtracting q up to four times
    __mmask32 tmp_m0;
    tmp_m0 = _mm512_cmple_epu16_mask(modulus16, tmp0);
    tmp0 = _mm512_mask_sub_epi16(tmp0, tmp_m0, tmp0, modulus16);
    tmp_m0 = _mm512_cmple_epu16_mask(modulus16, tmp0);
    tmp0 = _mm512_mask_sub_epi16(tmp0, tmp_m0, tmp0, modulus16);
    tmp_m0 = _mm512_cmple_epu16_mask(modulus16, tmp0);
    tmp0 = _mm512_mask_sub_epi16(tmp0, tmp_m0, tmp0, modulus16);
    ...
    tmp1 = _mm512_unpacklo_epi16(tmp0, zero);
    good = _mm512_cmplt_epi32_mask(tmp1, modulus8);
    _mm512_mask_compressstoreu_epi32((__m512i *)&a->coeffs[ctr], good, tmp1);
    offset = __builtin_popcount(good); // we get this many good (< modulus) values
    ctr += offset;

    tmp1 = _mm512_unpackhi_epi16(tmp0, zero);
    good = _mm512_cmplt_epi32_mask(tmp1, modulus8);
    _mm512_mask_compressstoreu_epi32((__m512i *)&a->coeffs[ctr], good, tmp1);
    offset = __builtin_popcount(good); // we get this many good (< modulus) values
    ctr += offset;
    ...
    if(pos > SHAKE128_RATE*nblocks-64)
    {
      nblocks=1;
      shake128_squeezeblocks((unsigned char *) buf,nblocks,state);
      pos = 0;
